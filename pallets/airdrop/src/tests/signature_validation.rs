...
const VALID_ICON_WALLET: &str = "0xee1448f0867b90e6589289a4b9c06ac4516a75a9";
...
fn siganture_validation_valid() {
	{
		...
		let icon_wallet = bytes::from_hex(VALID_ICON_WALLET).unwrap();
...
fn invalid_icon_address() {
	let icon_wallet = bytes::from_hex("0xee00080006009000000289a4b9c06ac451655555").unwrap();
	let account_id = AccountId32::from_str(VALID_ICE_ADDRESS).unwrap();
	let icon_signature = bytes::from_hex(VALID_ICON_SIGNATURE).unwrap();
...
fn mock_signature_validation() {
	...
	assert_ok!(samples::ACCOUNT_ID[0].verify_with_icon(&vec![], &vec![], &vec![]));
}
