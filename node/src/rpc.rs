...
use frost_runtime::{opaque::Block, AccountId, Balance, BlockNumber, Hash, Index};
...
/// Full client dependencies.
pub struct FullDepsForFrost<C, P, A: ChainApi> {
	/// The client instance to use.
	pub client: Arc<C>,
	/// Transaction pool instance.
	pub pool: Arc<P>,
...
pub fn create_full_frost<C, P, BE, A>(
	deps: FullDepsForFrost<C, P, A>,
	subscription_task_executor: SubscriptionTaskExecutor,
) -> jsonrpc_core::IoHandler<sc_rpc::Metadata>
where
	BE: Backend<Block> + 'static,
	...
	A: ChainApi<Block = Block> + 'static,
{
	...
	io.extend_with(EthApiServer::to_delegate(EthApi::new(
		...
		pool.clone(),
		graph,
		Some(frost_runtime::TransactionConverter),
		network.clone(),
		signers,
		overrides.clone(),
		backend.clone(),
